...
var executionAPIExample = (function() {
	...
	var SCRIPT_ID='1b5ece_I3H3I_odP0DhYrrGnaclsCT8NQucCbTiTjGYC-Uesr32oPLSDL'; // Apps Script script id
	var STATE_START=1;
	...
	function getAuthTokenCallback(token) {
		...
		if (chrome.runtime.lastError) {
			sampleSupport.log('No token aquired' + chrome.runtime.lastError.message); 
	...
	function sendDataToExecutionAPICallback(token) {
		...
		post({	'url': 'https://script.googleapis.com/v1/scripts/' + SCRIPT_ID + ':run',
				...
				'token': token,
				'request': {'function': 'createAssignments',
							'parameters': {}}
			});
	}
